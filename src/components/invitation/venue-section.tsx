...
export function VenueSection({ venue }: VenueSectionProps) {
  return (
    <section id="venue" className="section-padding" aria-labelledby="venue-heading">
      <SectionHeading
        eyebrow="Location"
        title="Kallisto Grand Marquee"
        description="Join us for an evening of love as we celebrate together."
      />
      ...
      <AnimateIn>
        <GlassCard className="mx-auto max-w-2xl text-center">
          ...
          <h3 id="venue-heading" className="sr-only">
            {venue.name}
          </h3>
          {(venue.address || venue.city) && (
            <p className="text-base text-muted-foreground">
              {venue.address}
              {venue.address && venue.city ? <br /> : null}
              {venue.city}
            </p>
          )}
          <a
            ...
            className={cn(
              ...
              "mt-6 inline-flex h-11 items-center gap-2 rounded-full border border-gold/35 bg-gradient-to-r from-gold-deep to-gold px-6 tracking-wide text-primary-foreground shadow-md transition-transform hover:scale-[1.03]"
            )}
          >
